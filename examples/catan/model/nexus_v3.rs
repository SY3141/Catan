//! Simplified heterogeneous GNN model for Catan (v3).
//!
//! Compared to nexus: 4 GNN layers (not 5), NF=23 (not 25), no edge features.
//! Road policy head uses HIDDEN*2 (node embedding sum + diff at endpoints).
...
static NEXUS_V3_GRAPH_DATA: OnceLock<NexusV3GraphData> = OnceLock::new();

fn nexus_v3_graph_data() -> &'static NexusV3GraphData {
    NEXUS_V3_GRAPH_DATA.get_or_init(|| {
...
#[derive(Module, Debug)]
pub struct CatanNexusModelV3<B: Backend> {
    // Input projections
...
pub fn init_nexus_v3<B: Backend>(device: &B::Device, num_aux_heads: usize) -> CatanNexusModelV3<B> {
    let graph = nexus_v3_graph_data();
...
impl<B: Backend> PolicyValueNet<B> for CatanNexusModelV3<B> {
